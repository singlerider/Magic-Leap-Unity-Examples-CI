...
namespace MagicLeap.Examples
{
    ...
    public class MLWebRTCExample : MonoBehaviour
    {
        ...
        private async Task CreateLocalMediaStream()
        {
            ...
            streamConfigs.Add(MLCamera.CaptureStreamConfig.Create(MLCamera.GetBestFitStreamCapabilityFromCollection(streamCapabilities, captureWidth, captureHeight, MLCamera.CaptureType.Video), outputFormat));
            if (MLCamera.IsCaptureTypeSupported(mlCamera, MLCamera.CaptureType.Preview) && useHWBuffers)
            {
                streamConfigs.Add(MLCamera.CaptureStreamConfig.Create(MLCamera.GetBestFitStreamCapabilityFromCollection(streamCapabilities, captureWidth, captureHeight, MLCamera.CaptureType.Preview), outputFormat));
            }
            ...
            {
                localVideoSource = MLWebRTC.MLCameraVideoSource.CreateLocal(mlCamera, captureConfig, out MLResult result, id, localVideoRenderer, useHWBuffers);
